...
M46 owns the M44/M45 local counters for one exact M43 prepared response. It never
invokes a writer and local completion remains explicitly distinct from transport.
"""
...
def _fail(code: str, message: str, *, transition_code: str | None = None, write_plan_code: str | None = None) -> None:
    raise InboundHttpResponseWriteSessionError(
        code, message, transition_code=transition_code, write_plan_code=write_plan_code
    )
...
def _replay_plan(value: InboundHttpResponseWritePlan) -> InboundHttpResponseWritePlan:
    ...
    for name in _NEGATIVE_FIELDS:
        if getattr(value, name, None) is not False:
            _fail("WRITE_SESSION_AUTHORITY_ESCALATION", "M44 plan promoted authority")
    try:
...
def _replay_prepared(value: PreparedInboundHttpReadResponse) -> PreparedInboundHttpReadResponse:
    ...
    for name in (
        ...
        if getattr(value, name, None) is not False:
            _fail("WRITE_SESSION_AUTHORITY_ESCALATION", "M43 prepared response promoted authority")
    try:
...
class InboundHttpResponseWriteSessionProgress:
    ...
    def __post_init__(self) -> None:
        ...
        if self.response_bytes <= 0 or self.bytes_written > self.response_bytes:
            raise ValueError("invalid response/write accounting")
        if plan.response_bytes != self.response_bytes or plan.bytes_written != self.bytes_written or plan.write_calls_completed != self.write_calls_completed:
        ...
        current = (
            "inbound-http-response-write-session-progress-v1", self.response_bytes,
            self.bytes_written, self.write_calls_completed, self.last_accepted_write_bytes,
            plan.integrity_snapshot, *(getattr(self, name) for name in _NEGATIVE_FIELDS),
        )
...
class CompletedInboundHttpResponseWriteSession:
    ...
    def __post_init__(self) -> None:
        ...
        if self.response_bytes != plan.response_bytes or self.bytes_written != self.response_bytes:
            raise ValueError("M46 completion byte accounting mismatch")
        if self.write_calls_completed != plan.write_calls_completed:
            raise ValueError("M46 completion call accounting mismatch")
        ...
        current = (
            "completed-inbound-http-response-write-session-v1", self.response_bytes,
            self.bytes_written, self.write_calls_completed, plan.integrity_snapshot,
            self.prepared_response_integrity, self.session_closed,
...
class BoundedInboundHttpResponseWriteSession:
    __slots__ = (
        "_transitioner", "_planner", "_prepared_response", "_response_identity",
        "_response_integrity", "_transition", "_transition_function", "_plan",
        "_plan_function", "_max_write_calls", "_max_write_bytes", "_write_calls_completed",
        "_bytes_written", "_current_plan", "_closed", "_state_witness",
    )
    ...
    def _validate_configuration(self) -> None:
        ...
        if (limits.max_write_calls, limits.max_write_bytes) != (self._max_write_calls, self._max_write_bytes):
            _fail("WRITE_SESSION_CONFIGURATION_DRIFT", "M45 write limits changed")
        planner_limits = self._planner.limits
        if (planner_limits.max_write_calls, planner_limits.max_write_bytes) != (self._max_write_calls, self._max_write_bytes):
            _fail("WRITE_SESSION_CONFIGURATION_DRIFT", "M44 write limits changed")
    ...
    def _authoritative_plan(self, calls: int, written: int) -> InboundHttpResponseWritePlan:
        ...
        _replay_prepared(self._prepared_response)
        if id(self._prepared_response) != self._response_identity or self._prepared_response.integrity_snapshot != self._response_integrity:
            _fail("WRITE_SESSION_STATE_DRIFT", "M46 prepared response binding changed")
        try:
            plan = self._plan(self._prepared_response, write_calls_completed=calls, bytes_written=written)
    ...
    def _open_witness(self) -> tuple[Any, ...]:
        return (
            "inbound-http-response-write-session-state-v1", False, self._response_identity,
            self._response_integrity, self._write_calls_completed, self._bytes_written,
    ...
    def _validate_state(self) -> None:
        ...
        if self._prepared_response is None or id(self._prepared_response) != self._response_identity:
            _fail("WRITE_SESSION_STATE_DRIFT", "M46 prepared response reference changed")
        if self._state_witness != self._open_witness():
            _fail("WRITE_SESSION_STATE_DRIFT", "M46 owned write state changed outside transition")
        ...
        if replayed.integrity_snapshot != self._current_plan.integrity_snapshot:
            _fail("WRITE_SESSION_STATE_DRIFT", "M46 current plan no longer matches authoritative M44 plan")
    ...
    def accept_write_count(self, accepted_write_bytes: int) -> InboundHttpResponseWriteSessionProgress:
        ...
        self._validate_configuration()
        if type(result) is not InboundHttpResponseWriteTransition:
        ...
        independently_planned = self._authoritative_plan(witnessed.write_calls_completed, witnessed.bytes_written)
        if independently_planned.integrity_snapshot != witnessed.next_plan.integrity_snapshot:
            _fail("WRITE_SESSION_NEXT_PLAN_DRIFT", "M45 next plan differs from independent M44 replay")
        # State mutates only after all independent checks pass.
        self._write_calls_completed = witnessed.write_calls_completed
        self._bytes_written = witnessed.bytes_written
        self._current_plan = independently_planned
        self._state_witness = self._open_witness()
        return InboundHttpResponseWriteSessionProgress(
            response_bytes=self._current_plan.response_bytes,
            bytes_written=self._bytes_written,
