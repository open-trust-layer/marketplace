"""Transport-free ownership of bounded inbound HTTP response-write progress.

M46 owns the exact M43 prepared response and the local M44/M45 write-accounting
state so normal callers cannot reset byte offsets or write-call counts between
transitions. It invokes no writer and performs no transport I/O. Local write
accounting is never evidence that response bytes reached a peer.
"""
...
_PREPARED_NEGATIVE_FIELDS: Final = (
...
_CLOSED_WITNESS: Final = ("inbound-http-response-write-session-state-v1", True)


class InboundHttpResponseWriteSessionError(RuntimeError):
    """Fail-closed M46 error with preserved M45/M44 reason metadata."""
...
def _fail(
    ...
    *,
    ...
    raise InboundHttpResponseWriteSessionError(
        code,
        message,
...
def _require_negative(value: object, fields: tuple[str, ...] = _NEGATIVE_FIELDS) -> None:
    for name in fields:
        if getattr(value, name, None) is not False:
            _fail("WRITE_SESSION_AUTHORITY_ESCALATION", "M46 observed promoted authority")
...
def _replay_plan(value: InboundHttpResponseWritePlan) -> InboundHttpResponseWritePlan:
    ...
    _require_negative(value)
    try:
        replayed = replace(value)
    except ValueError as exc:
        raise InboundHttpResponseWriteSessionError(
            "WRITE_SESSION_PLAN_DRIFT", "M44 plan failed integrity replay"
        ) from exc
    _require_negative(replayed)
    return replayed
...
def _replay_prepared(value: PreparedInboundHttpReadResponse) -> PreparedInboundHttpReadResponse:
    ...
    _require_negative(value, _PREPARED_NEGATIVE_FIELDS)
    try:
        replayed = replace(value)
    except ValueError as exc:
        raise InboundHttpResponseWriteSessionError(
            "WRITE_SESSION_RESPONSE_DRIFT", "M43 prepared response failed integrity replay"
        ) from exc
    if replayed.response_prepared is not True or replayed.transmitted is not False:
        _fail("WRITE_SESSION_RESPONSE_DRIFT", "replayed M43 authority changed")
    _require_negative(replayed, _PREPARED_NEGATIVE_FIELDS)
    return replayed


def _replay_transition(
    value: InboundHttpResponseWriteTransition,
) -> InboundHttpResponseWriteTransition:
    if type(value) is not InboundHttpResponseWriteTransition:
        _fail("WRITE_SESSION_TRANSITION_DRIFT", "M45 returned an unexpected transition type")
    # Check init=False authority facts before dataclasses.replace() can normalize them.
    _require_negative(value)
    try:
        replayed = replace(value)
    except ValueError as exc:
        raise InboundHttpResponseWriteSessionError(
            "WRITE_SESSION_TRANSITION_DRIFT", "M45 transition failed integrity replay"
        ) from exc
    _require_negative(replayed)
    return replayed
...
class InboundHttpResponseWriteSessionProgress:
    """Detached public M46 progress with no raw response bytes."""
    ...
    def __post_init__(self) -> None:
        ...
        if (
            plan.response_bytes,
            plan.bytes_written,
            plan.write_calls_completed,
        ) != (self.response_bytes, self.bytes_written, self.write_calls_completed):
        ...
        current = (
            "inbound-http-response-write-session-progress-v1",
            self.response_bytes,
            self.bytes_written,
            self.write_calls_completed,
            self.last_accepted_write_bytes,
        ...
        if self.integrity_snapshot is not None and self.integrity_snapshot != current:
            raise ValueError("M46 progress integrity snapshot mismatch")
...
class CompletedInboundHttpResponseWriteSession:
    """One-shot local M46 completion witness; never transmission evidence."""
    ...
    def __post_init__(self) -> None:
        ...
        if (
            self.response_bytes != plan.response_bytes
            or self.bytes_written != self.response_bytes
            or self.write_calls_completed != plan.write_calls_completed
        ):
            raise ValueError("M46 completion accounting mismatch")
        if (
            type(self.prepared_response_integrity) is not tuple
            or self.prepared_response_integrity != plan.prepared_response_integrity
        ):
            raise ValueError("M46 completion prepared-response binding mismatch")
        if self.session_closed is not True:
            raise ValueError("M46 completion MUST bind a closed source session")
        for name in _NEGATIVE_FIELDS:
            if getattr(self, name) is not False:
                raise ValueError("M46 completion promoted forbidden authority")
        current = (
            "completed-inbound-http-response-write-session-v1",
            self.response_bytes,
            self.bytes_written,
            self.write_calls_completed,
            plan.integrity_snapshot,
        ...
        if self.integrity_snapshot is not None and self.integrity_snapshot != current:
            raise ValueError("M46 completion integrity snapshot mismatch")
        if self.integrity_snapshot is None:
            object.__setattr__(self, "integrity_snapshot", current)


class BoundedInboundHttpResponseWriteSession:
    """Own M44/M45 write state without invoking any writer or transport."""
    ...
    def __init__(
        self,
        *,
        write_transitioner: BoundedInboundHttpResponseWriteTransitioner,
        ...
        self._transitioner = write_transitioner
        self._planner = planner
        self._prepared_response = prepared_response
        self._response_identity = id(prepared_response)
        self._response_integrity = prepared.integrity_snapshot
        self._transition_function = BoundedInboundHttpResponseWriteTransitioner.transition
        self._plan_function = BoundedInboundHttpResponseWritePlanner.plan
        ...
        self._max_write_calls = limits.max_write_calls
        self._max_write_bytes = limits.max_write_bytes
        self._write_calls_completed = 0
    ...
    def _validate_configuration(self) -> None:
        if (
            type(self._transitioner) is not BoundedInboundHttpResponseWriteTransitioner
            or type(self._planner) is not BoundedInboundHttpResponseWritePlanner
        ):
        ...
        if (
            getattr(self._plan, "__self__", None) is not self._planner
        ...
        # Also bind M45's own retained M44 helper, not only M46's independent replay helper.
        transitioner_plan = getattr(self._transitioner, "_plan", None)
        transitioner_plan_function = getattr(self._transitioner, "_plan_function", None)
        if (
            getattr(transitioner_plan, "__self__", None) is not self._planner
            or getattr(transitioner_plan, "__func__", None) is not self._plan_function
            or transitioner_plan_function is not self._plan_function
        ):
            _fail("WRITE_SESSION_CONFIGURATION_DRIFT", "M45 retained M44 plan binding changed")
        expected = (self._max_write_calls, self._max_write_bytes)
        limits = self._transitioner.write_limits
        planner_limits = self._planner.limits
        if (
            limits.max_write_calls,
            limits.max_write_bytes,
        ) != expected or (
            planner_limits.max_write_calls,
            planner_limits.max_write_bytes,
        ) != expected:
    ...
    def _authoritative_plan(self, calls: int, written: int) -> InboundHttpResponseWritePlan:
        ...
        if (
            id(self._prepared_response) != self._response_identity
            or replayed.integrity_snapshot != self._response_integrity
        ):
            _fail("WRITE_SESSION_STATE_DRIFT", "M46 prepared-response binding changed")
        try:
            plan = self._plan(
                self._prepared_response,
                write_calls_completed=calls,
                bytes_written=written,
            )
        except InboundHttpResponseWritePlanError as exc:
            self._validate_configuration()
            _fail(
                "WRITE_SESSION_PLAN_REJECTED",
                "M44 rejected M46 state",
                write_plan_code=exc.code,
            )
        self._validate_configuration()
        return _replay_plan(plan)

    def _open_witness(self) -> tuple[Any, ...]:
        return (
            "inbound-http-response-write-session-state-v1",
            False,
            self._response_identity,
            self._response_integrity,
            self._write_calls_completed,
            self._bytes_written,
            self._current_plan.integrity_snapshot,
    ...
    def _validate_state(self) -> None:
        if self._closed:
            if self._prepared_response is not None or self._state_witness != _CLOSED_WITNESS:
                _fail("WRITE_SESSION_STATE_DRIFT", "closed M46 state changed")
            return
        self._validate_configuration()
        if (
            self._prepared_response is None
            or id(self._prepared_response) != self._response_identity
            or self._state_witness != self._open_witness()
        ):
    ...
    def progress(self) -> InboundHttpResponseWriteSessionProgress:
        ...
        return InboundHttpResponseWriteSessionProgress(
            response_bytes=self._current_plan.response_bytes,
            bytes_written=self._bytes_written,
            write_calls_completed=self._write_calls_completed,
            last_accepted_write_bytes=0,
            plan=self._current_plan,
        )

    def accept_write_count(
        self, accepted_write_bytes: int
    ) -> InboundHttpResponseWriteSessionProgress:
        self._validate_state()
        if self._closed:
            _fail("WRITE_SESSION_CLOSED", "M46 session is closed")
        before_plan = self._current_plan
        before_calls = self._write_calls_completed
        before_bytes = self._bytes_written
        try:
            result = self._transition(
                self._prepared_response,
                write_calls_completed=before_calls,
                bytes_written=before_bytes,
                accepted_write_bytes=accepted_write_bytes,
            )
        except InboundHttpResponseWriteTransitionError as exc:
            self._validate_state()
            _fail(
                "WRITE_SESSION_TRANSITION_REJECTED",
                "M45 rejected write count",
        ...
        if (
            witnessed.accepted_write_bytes != accepted_write_bytes
            or witnessed.write_calls_completed != before_calls + 1
            or witnessed.bytes_written != before_bytes + accepted_write_bytes
        ):
            _fail("WRITE_SESSION_TRANSITION_DRIFT", "M45 transition accounting differs from supplied count")
        independent = self._authoritative_plan(
            witnessed.write_calls_completed, witnessed.bytes_written
        )
        ...
        # Mutate only after every exact replay and cross-check passes.
        self._write_calls_completed = witnessed.write_calls_completed
        self._bytes_written = witnessed.bytes_written
        self._current_plan = independent
        self._state_witness = self._open_witness()
        return InboundHttpResponseWriteSessionProgress(
            response_bytes=independent.response_bytes,
            bytes_written=self._bytes_written,
            write_calls_completed=self._write_calls_completed,
            last_accepted_write_bytes=accepted_write_bytes,
            plan=independent,
        )
    ...
    def take_completed(self) -> CompletedInboundHttpResponseWriteSession:
        ...
        completed = CompletedInboundHttpResponseWriteSession(
            ...
            write_calls_completed=self._write_calls_completed,
            plan=self._current_plan,
            prepared_response_integrity=self._response_integrity,
        )
        # Release the retained response before returning the detached completion witness.
        self.close()
        return completed

    def close(self) -> None:
        """Idempotently release the retained M43 response and permanently close."""
        self._prepared_response = None
        self._closed = True
        self._state_witness = _CLOSED_WITNESS
