...
class FederationNetworkPolicyAuditTests(unittest.TestCase):
    def test_security_audit_requires_m25_runtime_module_and_documentation(self):
        ...
        self.assertTrue(required_m25.issubset(set(_REQUIRED_SECURITY_FILES)))
        with tempfile.TemporaryDirectory() as temp_dir:
            findings: list[str] = []
            _audit_required_security_files(Path(temp_dir), findings)
            missing = set(findings)
            self.assertTrue(
                {
    ...
    def test_security_audit_accepts_complete_security_artifact_set(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            for relative in _REQUIRED_SECURITY_FILES:
                path = root / relative
                path.parent.mkdir(parents=True, exist_ok=True)
                path.write_text("security artifact\n", encoding="utf-8")
            findings: list[str] = []
